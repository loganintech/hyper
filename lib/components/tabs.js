...
//this component renders a list of tabs
export default class Tabs extends React.PureComponent {
  render() {
    ...
    //a tab title is displayed only when running on Mac OR when there are multiple tabs open
    return (
      <nav className={`tabs_nav ${hide ? 'tabs_hiddenNav' : ''}`}>
        {this.props.customChildrenBefore}
        {tabs.length === 1 && isMac ? <div className="tabs_title">{tabs[0].title}</div> : null}
        {tabs.length > 1 //when multiple tabs exist, each tab is passed information on where it is in the list of tabs. (eg. first, middle, last)
          ? [
