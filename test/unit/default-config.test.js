...
test('When checking the default config', t => {
    ...
    t.true( dc.config.cursorShape === 'BEAM'
    ||      dc.config.cursorShape === 'UNDERLINE'
    ||      dc.config.cursorShape === 'BLOCK'
    );
    ...
    t.true(dc.config.copyOnSelect === 'vertical' || dc.config.copyOnSelect === 'force');

    t.true(typeof dc.config.webGLRenderer === 'boolean');

    t.true(dc.plugins.isArray());

    t.true(dc.localPlugins.isArray());
